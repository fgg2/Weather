...
const _getStdin = require('get-stdin');

const _getStdin2 = _interopRequireDefault(_getStdin);

const _meow = require('meow');

const _meow2 = _interopRequireDefault(_meow);

const _needlessDisablesStringFormatter = require('./formatters/needlessDisablesStringFormatter');

const _needlessDisablesStringFormatter2 = _interopRequireDefault(_needlessDisablesStringFormatter);
...
const _resolveFrom2 = _interopRequireDefault(_resolveFrom);

const _standalone = require('./standalone');

const _standalone2 = _interopRequireDefault(_standalone);
...
const cli = (0, _meow2.default)(meowOptions, minimistOptions);

let formatter = cli.flags.formatter;
if (cli.flags.customFormatter) {
  const customFormatter = _path2.default.isAbsolute(cli.flags.customFormatter) ? cli.flags.customFormatter : _path2.default.join(process.cwd(), cli.flags.customFormatter);
...
if (cli.flags.customSyntax) {
  optionsBase.customSyntax = (0, _utils.getModulePath)(process.cwd(), cli.flags.customSyntax);
}
...
if (cli.flags.config) {
  ...
  // If none of the above work, well try a relative path starting
  // in `process.cwd()`.
...
Promise.resolve().then(() => {
  ...
  return (0, _getStdin2.default)().then(stdin =>
     (0, _lodash.assign)({}, optionsBase, {
       code: stdin,
     })
  );
}).then((options) => {
